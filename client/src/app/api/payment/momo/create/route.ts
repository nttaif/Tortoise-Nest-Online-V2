...
export async function POST(request: Request) {
  try {
    ...
    // Chuyển đổi số tiền thành số nguyên, loại bỏ mọi định dạng
    ...
    const requestBody = {
      ...
      amount: amountNumber,
      orderId: orderId,
      orderInfo: `Thanh toan khoa hoc: ${courseName}`,
    ...
    // Tạo chuỗi rawSignature từ các tham số được sắp xếp theo alphabet
    const rawSignature =
    `accessKey=${MOMO_ACCESS_KEY}` +
    `&amount=${amountNumber}` +
    `&extraData=` +
